...
**Limitações**: O MAE não consegue te indicar a **direção dos dados** por conta da utilização do módulo e por não elevar os dados ao quadrado, ou seja, não é possível dizer se os dados se distanciam dos valores reais positivamente ou negativamente. Além disso, ele é pouco sensível a outliers, ou seja, **erros grandes e pequenos tendem a ter a mesma influência** e isso pode ser um problema em situações onde erros grandes seriam inaceitáveis por exemplo.

### MSE

O **erro médio quadrático** segue a mesma lógica do MAE, calcula a distância entre o valor predito e o real, mas utilizando o **quadrado** ao invés do módulo, o que o torna **mais sensível a outliers** e penalizando mais **erros grandes**. Ele também é utilizado como **função de custo** (loss function) em modelos de regressão, como forma de **otimizar os parâmetros do modelo**, especialmente em **regressão linear** e em **redes neurais**, pois é sempre **diferenciável**, o que facilita minimizar a função utilizando **algoritmos de otimização**, como o gradiente descendente, por exemplo. Sendo assim, sua fórmula é:
...
**Limitações**: Comocitado anteriormente, o MSE tem uma **sensibilidade maior a outliers**, o que aumenta seu valor e pode indicar que o desempenho do modelo está pior quando isso não é verdade. Além disso, ele é **dependente da escala dos dados**, de maneira que modelos que preveem faixas de valor mais altas terão naturalmente um MSE maior, tornando a comparação entre diferentes bases de dados injusta. Para contornar esse problema, devemos **normalizar ou padronizar** os dados antes de realizar a conta. Outro problema a ser considerado é a **falta de informações direcionais** ao usá-lo, ou seja, nunca se sabe o modelo está prevendo valores grandes ou pequenos em relação à variável alvo

### $R^2$

$R^2$, também conhecido como coeficiente de determinação, é uma medida que **explica a proporção da variância** da variável dependente que pode ser explicada pelas variáveis independentes, ou seja, o quão bem os dados se **ajustam ao modelo**. Dessa forma, ele assume valores de zero até um, onde 1 significaria que os dados se encaixaram perfeitamente. Existem diversas maneiras de se calcular seu valor, vamos abordar duas neste texto. A primeira, se baseia em utilizar os **erros ao quadrado** e a **variância total do modelo**, de maneira que essa abordagem destaca o quão melhor o modelo de regressão funciona em comparação com simplesmente prever a média da resposta.

$R^2 = 1 - \frac{RSS}{TSS}$

onde RSS (residual sum of squares) é a soma dos erros ao quadrado e TSS (total sum of squares) é a variância total. Podemos escrever de maneira mais detalhada:
...
A segunda maneira de calcular o $R^2$ vamos utilizar o MSE e a **variância da variável dependente** (valor real).

$R^2 = 1 - \frac{MSE}{Var(y)}$

Essa abordagem é especialmente relevante ao comparar modelos em diferentes escalas ou unidades, porque normaliza o erro do modelo pela disseminação geral de dados.

**Interpretação**: Assim como o 𝑀𝐴𝐸, o $R^2$ tem uma interpretação que **varia de acordo com o contexto (dados)**, então nem sempre um valor alto é interessante ou necessário e nem valores baixos preocupantes. Mas, de forma geral, um valor alto explicaria grande parte da variabilidade dos dados e um valor pequeno o exato oposto.
...
Se nosso critério de aceitação do modelo for pior que 4.65 minutos de erro, então esse modelo é bom.
