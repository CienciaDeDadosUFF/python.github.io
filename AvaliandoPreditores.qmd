...
Os dados são valores quantitativos que foram extraidos de 4601 e-mails e associam o e-mail a uma classificação: `spam` ou `nom-spam`. Há 2788 spams e 1813 e-mails normais, isto é, 60.6% para spam e 39.4% para nonspam aproximadamente.
...
O **erro médio absoluto** (mean absolute error) mede a **distância absoluta entre os valores observados e os preditos pelo modelo**, ou seja, o **erro**. Para calculá-lo, obtemos a média das diferenças absolutas entre os valores preditos e os valores observados, como mostrado na fórmula a seguir:
...
**Interpretação**: Como comentado anteriormente, o calculo é baseado na distância entre os dados previstos e observados, portanto, quanto **menor** o valor do MAE, melhor o desempenho do modelo pois isso indica uma **maior proximidade** entre esses valores.

**Limitações**: O MAE é pouco sensível a outliers, ou seja, **erros grandes e pequenos tendem a ter a mesma influência** e isso pode ser um problema em situações onde erros grandes seriam preocupantes por exemplo.

### MSE

O **erro médio quadrático** (mean squared error) segue a mesma lógica do MAE, calcula a distância entre o valor predito e o observado, mas utilizando o **quadrado** ao invés do módulo, o que o torna mais **sensível a outliers** penalizando mais **erros grandes**. Ele também pode ser utilizado como **função de custo** (loss function) em modelos de regressão, como forma de **otimizar os parâmetros do modelo**, especialmente em **regressão linear** e em **redes neurais**, pois costumar ser **diferenciável**, o que facilita minimizar a função utilizando **algoritmos de otimização**, como o **gradiente descendente**, por exemplo. O MSE é dado por:
...
**Interpretação**: Assim como o MAE, quanto **menor** o MSE de um modelo, melhor. Contudo, sua interpretação requer cuidado, pois, para ser considerado grande ou pequeno, devemos analisar o contexto, além de **não estar na mesma unidade dos dados**.

**Limitações**: Como citado anteriormente, o MSE tem uma **sensibilidade maior a outliers**, o que pode aumentar seu valor e indicar que o desempenho do modelo está pior do que aparenta. Além disso, ele é **dependente da escala dos dados**, de maneira que modelos que preveem faixas de valor mais altas terão naturalmente um MSE maior. Para contornar esse problema, podemos **normalizar ou padronizar** os dados antes de observadoizar o cálculo.

### $R^2$

O $R^2$, também conhecido como coeficiente de determinação, mede **a proporção da variância** da variável resposta que é explicada pelas variáveis explicativas. O cálculo do $R^2$ utiliza os **erros ao quadrado** e a **variância dos dados observados**.
...
**Interpretação**: O $R^2$ assume valores no intervalo entre 0 e 1, de maneira que um $R^2$ igual a 0 indica que o modelo não consegue explicar a variância da variável resposta, enquanto 1 sugere que o modelo a explica perfeitamente. Contudo, valores de $R^2$ altos devem ser observados com cautela, pois podem indicar **overfitting**, comprometendo a generalização do modelo para novos dados.

**Limitações**: É importante lembrar que as variáveis explicativas explicam a variância da variável resposta mas não necessariamente sejam a causa dela. Além disso, o $R^2$ não indica se as previsões são precisas.
