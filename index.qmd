...
Porém como diremos para o Python fazer isso? E aonde que ele dirá esse "Hello World"? Ele não tem uma interface visual que nem o Excel que você coloca numa célula uma fórmula que o excel executa.
...
O `print` imprime na tela o conteúdo entre parênteses na tela. Se o conteúdo for textual precisa estar entre aspas. Números podem ser imprimidos diretamente sem o uso de aspas. O `print` por configuração padrão imprime as mensagens pulando uma linha depois. Para mudar essa configuração em um print específico, no final do comando, antes de fechar os parênteses, coloque `, end=" "`. Agora para esse `print` específico uma linha não será pulada no final da execução:
