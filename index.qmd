...
```
Hello World
Esse é meu terceiro programa em Python
```
:::

# Variáveis

**AJUDA PARA REESCREVER: Na escola quebramos a cabeça em matemática para resolver uma equação. A professora Márcia que todos tivemos passa uma expressão: 4 + x = 7. Mesmo como crianças sabíamos que para o 4 virar 7 o x precisaria ser um 3, o x armazena o valor 3. Na programação existem inúmeras vantagens de se usar variáveis.**

Suponha que queremos imprimir uma mesma mensagem inúmeras vezes:
...
Agora ao invés de imprimir "Ciência de dados!" queremos imprimir "O meu filme favorito é troll 2!". Para fazer isso teríamos que trocar cada uma das vezes o texto dentro do print pelo texto desejado. Existe alguma forma mais rápida de fazer isso: usando variáveis. Podemos criar uma variável x que armazena o texto a ser impresso.
...
**TIRAR: A escolha do uso da nomenclatura `x` para realizar a tarefa é apenas para seguir a didática da professora Márcia. Porém, idealmente** 

A nomeação de variáveis se faz baseado no uso dela, isto é, idealmente criamos variáveis com nomes elusivos. Nesse caso, ao invés de `x` usar `texto`. Isso facilita em projetos maiores quando várias pessoas trabalham juntas ou quando você precisa rever um código antigo.
...
O Python possui algumas regras para nomeação de variáveis. Assim como todas as linguagens de programação, existem alguns comandos que fazem uma tarefa, como o `print`. Esses comandos ou como chamamos na programação, funções, possuem o status de palavra reservada, que não podem ser usadas para outras coisas. Outras regras de nomeação de variáveis no Python são que as variáveis não podem começar com números (podem ter números, só não como primeiro caracter), não podem ter "." e nem espaços.

## Tipos de Variáveis (TIRAR SUBSEÇÃO)

No exemplo anterior, `texto` é uma variável não numérica, para definir um valor numérico basta associar a variável ao valor numérico desejado usando `=`.

``` python
x = 4
```

Note que para valores numéricos não há a necessidade de colocar o valor entre aspas. Essa diferença se dá no tipo do dado atribuído, um conceito importantíssimo em linguagens de programação. Enquanto "O meu filme favorito é troll 2!" é um texto, 4 é um número.

O que define se um dado é do tipo texto ou numérico é se ele está entre `" "` ou não, respectivamente. Mas então qual a diferença?
...
Pelo print parece a mesma coisa. A diferença é no comportamento que essas variáveis vão ter no programa. Qual será o resultado de x + x? E de y + y? 
...
Para textos o operador `+` concatena os termos, já para valores numéricos realiza soma dos termos.

Quando criamos uma variável, estamos criando uma instância de um tipo, ou instanciando um tipo. **SERÁ QUE USAMOS ESTA TERMOLOGIA?**

Cada **tipo** possui características próprias, que vão além de apenas comportamentoS frente a operadores. **VERIFICAR: Forma de instanciar, memória do computador usada, uso em funções, cada uma dessas características definem um tipo.**

## Função `type`

Antes de entrar mais especificamente em cada tipo, vamos apresentar uma ferramenta importante, a função `type` que verifica o tipo de uma variável. Em conjunto com o `print` imprime o tipo da variável.
...
**TIRA?? Repare que uma função pode ser usada dentro de outra. Falaremos mais de funções em breve.**

Sabemos que se trata de um valor inteiro, afinal, nós criamos essa variável. Porém, como muitas coisas na programação, aumentando a escala saber o tipo específico de uma variável em um programa pode ser um pouco mais difícil.
...
**TIRAR??Sempre que colocamos números sem estar entre aspas estamos instanciando um int:**
...
Abreviação para *string*, que traduzido do inglês significa fio, corda ou barbante, mas quando usado na programação significa cadeia, uma cadeia de caracteres. Linguagens de programação tradicionalmente possuem o tipo **char**, que é uma abreviação de characters que traduzido do inglês é caracteres. Esse tipo armazena apenas um caractere textual, o conjunto desses caracteres é chamado de *string*. No python, para simplificar, todo dado textual é um `str`.

**TIRAR?? Instanciação**

Para definir um `str` colocamos o texto entre aspas simples ou duplas.Podemos também usar `""" """`, usado quando queremos escrever um texto em mais de uma linha de código no Python. 
...
Note que para usar aspas duplas dentro de uma *string* precisamos escrever a *string* entre aspas simples para não terminar precocemente a *string*, o que resultaria em um erro. O contrário também serve para aspas simples como no apóstrofe da frase em inglês: `"That's life."`.

### float

Equivalente aos números racionais, `float` vem do padrão de **ponto flutuante**, uma técnica da computação para representar números com casas decimais no sistema binário. Enquanto os números inteiros têm uma tradução mais simples para o binário, as frações decimais muitas vezes resultam em sequências binárias infinitas, similar à dízima periódica. O padrão `float` é uma forma eficiente, mas aproximada, de lidar com isso.

**TIRAR?? Instanciação**

Para definirmos um `float` colocando os números com a parte decimal explicita. Lembrando que a linguagem usa o sistema americano, que utiliza o ponto como separador decimal.
...
Note que o Python por padrão corta os zeros à direita, ele só mantém o primeiro decimal obrigatório.
...
Para definição basta escrever *True* ou *False*, sem aspas. Outra opção é fazer um teste com operadores lógicos.
...
Antes de introduzirmos o conceito de listas (`list`) em Python, é útil entender o conceito de vetor na computação. Um vetor é uma estrutura de dados que armazena uma coleção ordenada de itens do mesmo tipo. Em Python, a estrutura nativa mais similar a um vetor é o `array` (do módulo array).

**TALVEZ COLOCAR EXEMPLO DO ARRAY**

As listas são mais flexíveis: elas podem armazenar itens de tipos diferentes e são a estrutura mais comum e versátil para guardar sequências de elementos.

**TIRAR?? Instanciação**

Para definir uma lista no Python usamos colchetes, e separamos diferentes elementos com uso de vírgulas:
...
**TIRAR?? Existem outros tipos de vetores além das listas no próprio python: `tuple`, `set`, `dict`. Eles são usados em certas situações mais específicas. Para se aprofundar no python são indispensáveis. Quando chegarmos no uso de ferramentas mais diretas para ciência de dados falaremos um pouco mais sobre isso.**
...
O primeiro índice é referente a posição da lista na matriz e o segundo índice é a posição do elemento específico na lista desejada. É equivalente ao conceito de colunas e linhas de uma matriz. Lembrando que o primeiro índice de uma lista é 0 e não 1.

Pode-se também fazer uma lista de matrizes, que se acessa usando 3 colchetes. Matrizes tem dimensões infinitas no Python, mas para ciência de dados comumente usamos apenas 2 dimensões: colunas e linhas. Para computação gráfica, por exemplo, podemos usar mais dimensões e matrizes passam a ser chamadas de tensores. 
**TIRAR???Isso não é importante para ciência de dados, pelo menos não agora (uma pequena antecipação de cenas dos próximos capítulos).**

### function

`function`, que do inglês significa função, possui conceito semelhante ao da matemática, realizar tarefas pré determinadas a partir de entradas especificadas.

O `print()` é um exemplo de função. Ele recebe como argumento um texto e imprime na tela. Outro exemplo é a função `type()` que recebe uma variável qualquer e retorna o seu tipo.

Existe uma nomenclatura própria para trabalhar com funções, o que a função recebe de entrada é o que chamamos de argumentos e o que ela cria a partir desse argumento é o retorno ou resultado. Uma função pode ter mais de um argumento, que são separados por vírgulas. Algumas funções não possuem resultado, o print() por mais que imprima na tela o texto, ele não gera retorno específico, ele é uma função bem especial, por isso falamos dela antes de qualquer outra.

**TIRAR????**
`nomeDaFunção(argumento1, argumento2, …, argumentoN)`

Lembra que para imprimir duas informações diferentes em uma mesma linha precisávamos escrever um `, end=""` depois do texto no `print()`? O print tem diversos argumentos, o `end` é um dos argumentos, por padrão é um `"\n"` que faz com que o computador pule uma linha. Argumentos que possuem padrão, se quisermos alterar, precisamos especificá-los, já que não é obrigatório para o funcionamento da função. 

**TIRAR TODA SEÇÃO??? Chamar uma função X Referenciar uma função**
...
**Esse código apenas exibe o tipo da variável print, que é uma função. Essa forma de representar funções é útil em códigos mais complexos que usam funções customizadas dentro de outras funções.Na referenciação, funções funcionam como qualquer outra variável.**

**TIRAR?? Instanciação**
Diferentes dos demais tipos, para definir uma função é necessário defini-la através do comando `def`. 
...
**TALVEZ TIRAR? Como pode ver é diferente dos demais tipos, usa uma palavra reservada para definir, o def. Para funções, invés de dizer que estamos instanciando, dizemos que estamos definindo-a. Em breve falaremos mais sobre como definir uma função, por enquanto é importante apenas entender que existem algumas funções padrões no python e que podemos criar mais se desejarmos.**

### Objetos

PRECISO DE AJUDA PARA AVALIAR ESTA SEÇÃO. TAMBÉM HÁ REFERENCIAS AOS OBJETOS NO CADERNO 2.
...
Os operandos **operadores** são sinalizações para o Python para realizar alguma operação sobre variáveis/valores. Muitos são idênticos ou semelhantes aos operadores matemáticos. 

Diferentes tipos possuem diferentes comportamentos em relação aos operadores.

## Adição (+) e Subtração (-)

O operador + é o operador da adição e - da subtração.

### int e float

Em relação a `int` e `float` funcionam igual à matemática:
...
Se somar/subtrair um `int` com um `float` a resposta será sempre um `float`. 
...
O Python transforma o valor de True para o inteiro 1 e False para o inteiro 0 e soma como se fossem inteiros. 
...
Podemos usar multiplicação em strings. É como se fosse várias operações de adição em sequência, e funciona igual a adição de strings normal. Não há divisão de strings no Python.
...
## Divisão interira (//) e Módulo (%)

`//` é o operador para divisão inteira e `%` é o operador módulo (resto da divisão). Funciona apenas para valores inteiros e `float`.

25 amigos foram jogar futebol na praça, querem montar equipes de 11 e as pessoas restantes ficam "de fora" e entram quando o jogo acabar substituindo alguém do time perdedor. Quantos ficaram "de fora" e quantas equipes foram formadas?
...
Funciona igual a matemática, apenas o símbolo que é novo. Apenas para inteiros e `float`. Assim como na matemática, exponenciação pode ser radiciação se usando números menores que 1 como expoentes.
